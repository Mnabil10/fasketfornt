...
export default function RequireAdminOrStaff({ children }: { children: React.ReactNode }) {
  const { isAuthed, isAdmin, isStaff, isOps, isFinance, isProvider, isDriver, user } = useAuth();
  ...
  const allowed =
    ...
    isProvider ||
    isDriver ||
    (user?.role ? ["ADMIN", "STAFF", "OPS_MANAGER", "FINANCE", "PROVIDER", "DRIVER"].includes(user.role) : false);
